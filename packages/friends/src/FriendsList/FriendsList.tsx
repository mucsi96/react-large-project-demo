import { ApiError, Button, Spinner, withErrorBoundary } from 'core';
import React, { FC } from 'react';
import styles from './FriendsList.module.scss';
import { useFriends } from './useFriends';

export const FriendsList: FC = withErrorBoundary(
  () => {
    const {
      friends = [],
      isLoading,
  ...
  (error: ApiError) =>
    `${error.response?.error?.message ?? ''} Status: ${error.status ?? ''}`
);
